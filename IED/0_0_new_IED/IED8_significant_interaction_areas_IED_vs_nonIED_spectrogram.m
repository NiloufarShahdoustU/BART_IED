% ied vs non-ied spectrograms for every significant ied8 interaction area
...
outputFolder = ...
    ['D:\Nill\code\BART\IED\0_0_new_IED\' ...
     'IED8_significant_interaction_areas_IED_vs_nonIED_spectrogram\'];
...
for oo = 1:length(outcomeNames)
    ...
    useRows = ...
        ...
        logical(resultTable.significantUncorrected_IEDxExpectedReward) & ...
        isfinite(double(resultTable.beta_IEDxExpectedReward));
...
if isempty(allResults)
    error('no significant ied8 interaction areas were found');
end
...
for pt = 1:length(fileList)
    ...
    expectedReward = load_expected_reward( ...
        modelFileList, modelingFolder, participantID);
...
% make a separate pdf for every significant outcome-area pair
...
for rr = 1:length(allResults)
    ...
    ax1 = nexttile(layout);
    imagesc(ax1, spectrogramTime, spectrogramFrequencies, meanIED);
    set(ax1, ...
        'YScale', 'log', ...
        'YDir', 'normal', ...
        'YLim', [5 200], ...
    ...
    ax2 = nexttile(layout);
    imagesc(ax2, spectrogramTime, spectrogramFrequencies, meanNonIED);
    set(ax2, ...
        'YScale', 'log', ...
        'YDir', 'normal', ...
        'YLim', [5 200], ...
    ...
    ax3 = nexttile(layout);
    differenceImage = imagesc(ax3, ...
        spectrogramTime, spectrogramFrequencies, meanDifference);
    set(differenceImage, 'AlphaData', alphaMask);
    set(ax3, ...
        'YScale', 'log', ...
        'YDir', 'normal', ...
        'YLim', [5 200], ...
    ...
    title(layout, sprintf([ ...
        'IED8 %s IED x expected reward | %s | ' ...
        'baseline %.1f to %.1f s | ' ...
    ...
    % make a clean file name, for example Amygdala_IT_spectrogram.pdf

    safeAreaName = regexprep( ...
        char(areaName), '[^a-zA-Z0-9]+', '_');

    safeAreaName = regexprep( ...
        safeAreaName, '^_+|_+$', '');

    if isempty(safeAreaName)
        safeAreaName = 'Unknown';
    end

    areaPDF = fullfile(outputFolder, sprintf( ...
        '%s_%s_spectrogram.pdf', ...
        safeAreaName, char(outcome)));

    if exist(areaPDF, 'file')
        delete(areaPDF);
    end

    exportgraphics(fig, areaPDF, ...
        'ContentType', 'image', ...
        'Resolution', 220);

    numberOfSavedPDFs = numberOfSavedPDFs + 1;
...
function expectedReward = load_expected_reward( ...
    ...
    participantIDlower = lower(char(participantID));
    escapedID = regexptranslate('escape', participantIDlower);
    tokenPattern = ...
    ...
    if isempty(matchedRows)
        matchedRows = find(contains( ...
            lower(modelNames), lower(string(participantID))));
    end
