...
outputFolderName = ...
    'D:\Nill\code\BART\IED\0_0_new_IED\IED8_3_Cox_expected_reward_by_brain_area_occurrence\';
...
function [permutationPValue, nSuccessfulPermutations] = ...
    runMechanisticInteractionPermutationTest( ...
        ...
        observedInteractionBeta, settings)
    ...
    for permutationNumber = 1:nPermutations
        ...
        permutationBetas(permutationNumber) = ...
            fitMechanisticInteractionCoefficient( ...
    ...
    if nSuccessfulPermutations == 0 || ...
            ~isfinite(observedInteractionBeta)
        permutationPValue = NaN;
        return;
    end

    permutationPValue = (1 + sum( ...
        abs(validPermutationBetas) >= ...
        abs(observedInteractionBeta))) ./ ...
...
function permutedIEDoccurrence = permuteIEDTrialAssignments( ...
    ...
    if isempty(validTrialNumbers)
        permutedIEDoccurrence = zeros( ...
            0, size(areaIEDoccurrence, 2));
        return;
    end
    ...
    % Move each trial's complete IED pattern as one unit to another valid
    % trial from the same participant. IED-free trials are also preserved.
...
function interactionBeta = fitMechanisticInteractionCoefficient( ...
    ...
    try
        ...
        if length(beta) >= interactionIndex && ...
                isfinite(beta(interactionIndex))
            interactionBeta = beta(interactionIndex);
        end
    catch
        interactionBeta = NaN;
    end
