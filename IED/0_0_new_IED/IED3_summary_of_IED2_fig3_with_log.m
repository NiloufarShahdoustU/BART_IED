...
for pt = 1:length(files)
    ...
    for k = 1:3
        ...
        if k == 3
            model = fitglm(T, 'y ~ logX', 'Distribution', 'binomial', 'Link', 'logit');
        ...
        newTrials = table(repmat(ptID, n, 1), repmat(comparison(k), n, 1), ...
            repmat(xName(k), n, 1), repmat(yName(k), n, 1), x, logX, y, ...
            'VariableNames', {'patientID', 'comparison', 'xMeasure', 'yMeasure', ...
            'rawX', 'logX', 'y'});
...
for k = 1:3
    ...
    if k == 3
        model = fitglme(rows, 'y ~ logX + (1 | patientID)', ...
...
ylabel(sprintf(['Model slope\nRT/IT: seconds per log10(IED rate + 1); ' ...
    'BR: log-odds per log10(IED rate + 1)']), 'FontSize', 12, 'FontWeight', 'bold');
