% Channel-specific Cox analysis for IT, RT, and BR
% Tests channels first, then summarizes significant channels by brain area.

clear; clc; close all;

inputFolderName = 'D:\Nill\data\BART\0_0_new_IED\IED1_find_number_of_IEDs\';
outputFolderName = 'D:\Nill\code\BART\IED\0_0_new_IED\IED6_Cox_IT_RT_BR_postIED\';
...
settings.channelFDRLevel = 0.05; % exploratory channel-discovery FDR
...
for analysisType = analysisTypes
    ...
    fprintf('============================================================\n');
    R = runChannelAnalysis(inputFolderName,analysisType,settings);
...
function results = runChannelAnalysis(inputFolder,analysisType,S)
...
for ff = 1:numel(files)
    ...
    elseif analysisType=="IT"
        ...
        n=min([numel(RT),numel(IT),numel(bank),numel(control),numel(balloon)]);
    end
        ...
        if nIED<S.minimumIEDsPerChannel
            ...
            for tt=1:numel(trials)
                ...
                observedRows{tt}=makeRows(tr,duration(tr),endpoint(tr), ...
                    balloon(tr),channelIED,Fs,window);
            end
            ...
            elseif nOut<S.minimumEventsOutsideWindow
                ...
                try
                    [beta,se,z]=fitChannelCox(CP,S.minimumEventsInsideWindow);
                    ...
                    for permutationIndex=1:S.numberOfPermutations
                        ...
                        for trialIndex=1:numel(trials)
                            ...
                            permutedRows{trialIndex}=makeRows(tr,duration(tr), ...
                                endpoint(tr),balloon(tr),shiftedIED,Fs,window);
                        end
                        permutedCP=vertcat(permutedRows{:});
                        try
                            permBeta(permutationIndex)=fitChannelCox( ...
...
for pp=1:numel(patients)
    idx=results.patientID==patients(pp) & results.status=="fitted" & isfinite(results.pValue);
    if any(idx)
        results.pValueFDR(idx)=BH(results.pValue(idx));
        results.significantChannel(idx)=results.pValueFDR(idx)<S.channelFDRLevel;
    end
end
results=sortrows(results,{'significantChannel','pValueFDR'},{'descend','ascend'});
end
...
T=table(strings(0,1),strings(0,1),zeros(0,1),zeros(0,1),strings(0,1), ...
    ...
    'nEventsInsideWindow','nEventsOutsideWindow','beta_logHazard','standardError', ...
    'zStatistic','pValue','pValueFDR','hazardRatio','hazardRatioCILow', ...
    'hazardRatioCIHigh','significantChannel'});
end
...
if isempty(R)
    S=table(strings(0,1),strings(0,1),zeros(0,1),zeros(0,1),zeros(0,1), ...
        ...
        'nParticipants','medianHazardRatio'}); return
end
[G,type,area]=findgroups(R.analysisType,R.anatomicalArea);
S=table(type,area,splitapply(@numel,R.pValue,G), ...
    splitapply(@(x)numel(unique(x)),R.patientID,G), ...
    splitapply(@median,R.hazardRatio,G),'VariableNames', ...
    {'analysisType','anatomicalArea','nSignificantChannels','nParticipants','medianHazardRatio'});
...
for i=1:3
    ...
    xlabel('Number of significant channels'); title(types(i)); grid on;
...
function T=emptyCP()
T=table(zeros(0,1),zeros(0,1),false(0,1),false(0,1),zeros(0,1),zeros(0,1), ...
    'VariableNames',{'tStart','tStop','censored','eventAtStop','postIED','balloonColorCode'});
end
...
warningCleanup=onCleanup(@()restoreWarningStates( ...
    singularState,nearSingularState)); %#ok<NASGU>
...
if ~all(isfinite([beta,se,z])) || se<=0
    error('Cox fit returned an unstable estimate.');
end
end
