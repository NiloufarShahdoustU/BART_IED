...
outputFolderName = ...
    'D:\Nill\code\BART\IED\0_0_new_IED\IED8_cox_expected_reward\';
...
function result = runOverallMechanisticCoxAnalysis( ...
    ...
    if range(countingProcessData.postIED) == 0
        error('%s: post-IED indicator has no variation.', codeChar);
    end
    ...
    interactionIndex = find( ...
        predictorNames == "post_ied_x_expected_reward", 1);
    ...
    fprintf('expected-reward slope outside post-IED window = %.6f\n', ...
        slopeOutsideIED);
    fprintf('expected-reward slope inside post-IED window = %.6f\n', ...
        slopeInsideIED);
    ...
    if permutationP(interactionIndex) < 0.05
        if beta(interactionIndex) > 0
            ...
            fprintf(['conclusion: expected reward has a significantly ' ...
                'more negative log-hazard slope after an IED.\n']);
        end
    else
        fprintf(['conclusion: no significant evidence that the ' ...
            'expected-reward slope changes after an IED.\n']);
    end
...
function interpretation = createPredictorInterpretations( ...
    ...
    for ii = 1:length(predictorNames)
        switch predictorNames(ii)
            case "post_ied_indicator"
                interpretation(ii) = ...
                    "Post-IED hazard difference when expected reward equals zero.";
            case "expected_reward"
                interpretation(ii) = ...
                    "Expected-reward log-hazard slope outside post-IED windows.";
            case "post_ied_x_expected_reward"
                interpretation(ii) = ...
                    "Change in the expected-reward log-hazard slope after an IED.";
            otherwise
                interpretation(ii) = ...
    ...
    if config.code == "IT"
        interpretation(predictorNames == "expected_reward") = ...
            "Expected-reward effect on action hazard outside post-IED windows.";
    elseif config.code == "RT"
        interpretation(predictorNames == "expected_reward") = ...
            "Expected-reward effect on response hazard outside post-IED windows.";
    else
        interpretation(predictorNames == "expected_reward") = ...
            "Expected-reward effect on banking hazard outside post-IED windows.";
    end
...
function effectTable = createExpectedRewardEffectTable( ...
    ...
    for gg = 1:nGrid
        ...
        % Reference is: no post-IED exposure at mean expected reward.
...
function fig = createCombinedExpectedRewardVisualization( ...
    ...
    applyBoldFigureText(fig);

    % sgtitle can be touched by global styling, so enforce it again.
    set(mainTitle, ...
        'FontName', 'Arial', ...
        'FontSize', 14, ...
        'FontWeight', 'bold', ...
...
function labels = humanReadablePredictorLabels(predictorNames)
    ...
    for ii = 1:length(predictorNames)
        switch predictorNames(ii)
            case "post_ied_indicator"
                labels(ii) = "post-IED indicator";
            case "expected_reward"
                labels(ii) = "expected reward";
            case "post_ied_x_expected_reward"
                labels(ii) = "post-IED x expected reward";
...
function plotExpectedRewardEffectPanel(ax, result, config)
    ...
    noIEDLine = plot( ...
        ...
        'DisplayName', 'outside post-IED window');
    ...
    postIEDLine = plot( ...
        ...
        'LineWidth', 2.3, ...
        'Color', config.color, ...
        'DisplayName', 'inside post-IED window');
    ...
    legendHandle = legend( ...
        ax, ...
        [noIEDLine, postIEDLine], ...
        {'outside post-IED window', 'inside post-IED window'}, ...
        'Location', 'best', ...
        'Box', 'off', ...
        'FontName', 'Arial', ...
        'FontSize', 12, ...
...
function applyBoldFigureText(fig)

    allAxes = findall(fig, 'Type', 'axes');

    for ii = 1:length(allAxes)
        currentAxis = allAxes(ii);
        ...
        set(currentAxis.Title, ...
            'FontName', 'Arial', ...
            'FontSize', 14, ...
            'FontWeight', 'bold', ...
            'Color', 'k');

        set(currentAxis.XLabel, ...
            'FontName', 'Arial', ...
            'FontSize', 12, ...
            'FontWeight', 'bold', ...
            'Color', 'k');

        set(currentAxis.YLabel, ...
            'FontName', 'Arial', ...
            'FontSize', 12, ...
            'FontWeight', 'bold', ...
            'Color', 'k');

        set(currentAxis.ZLabel, ...
            'FontName', 'Arial', ...
            'FontSize', 12, ...
            'FontWeight', 'bold', ...
            'Color', 'k');
    ...
    for ii = 1:length(allText)
        ...
        if allText(ii).FontSize < 12
            allText(ii).FontSize = 12;
        end
    end

    allLegends = findall(fig, 'Type', 'Legend');

    for ii = 1:length(allLegends)
        set(allLegends(ii), ...
            'FontName', 'Arial', ...
            'FontSize', 12, ...
            'FontWeight', 'bold', ...
