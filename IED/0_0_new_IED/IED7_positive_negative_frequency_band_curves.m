% positive and negative ied7 frequency band curves
...
ied7ResultsFile = ...
    ['D:\Nill\code\BART\IED\0_0_new_IED\' ...
     'IED7_Cox_IT_RT_BR_postIED_by_brain_area\' ...9
     'IT_RT_BR_brain_area_cox_all_results.mat'];

outputFolder = ...
    ['D:\Nill\code\BART\IED\0_0_new_IED\' ...
     'IED10_positive_negative_frequency_band_curves\'];
...
outputPDF = fullfile(outputFolder, ...
    'IED7_positive_negative_frequency_band_curves.pdf');
...
% get positive and negative areas
...
for oo = 1:4
    ...
    if outcome == "PR"
        resultTable = ied7.BRResults;
        beta = resultTable.betaPop_logHazard;
        significant = resultTable.significantPopUncorrected;
    else
        resultTable = ied7.(char(outcome + "Results"));
        beta = resultTable.beta_logHazard;
        significant = resultTable.significantUncorrected;
    end
