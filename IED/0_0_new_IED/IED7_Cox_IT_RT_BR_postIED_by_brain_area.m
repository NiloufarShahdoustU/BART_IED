...
settings.numberOfPermutations = 10000;
settings.permutationSeed = 20260713;
settings.minimumSuccessfulParticipantFraction = 0.80;
...
function [areaResults, areaModels] = runLoggedBrainAreaAnalysis( ...
    ...
    fprintf('FDR-significant areas: %d\n', ...
        sum(areaResults.significantFDR));
    fprintf('Saved: %s\n', resultsOutputFile);
    fprintf('Saved: %s\n', modelOutputFile);
    fprintf('Saved: %s\n', textOutputFile);
...
function [areaResults, areaModels] = runBrainAreaAnalysis( ...
    ...
    for aa = 1:length(allAreas)
        ...
        robustZPrimary = NaN;
        pValuePrimary = NaN;
        waldPValuePrimary = NaN;
        nSuccessfulPermutationParticipants = 0;
        jackknifePseudoValues = NaN(0, 1);
        leaveOneParticipantOutBeta = NaN(0, 1);
        betaCILow = NaN;
        betaCIHigh = NaN;
        hazardRatio = NaN;
        ...
        if status == "fitted"
            ...
            try
                ...
                waldPValuePrimary = clusterRobustP(1);

                analysisSeedOffset = getAnalysisSeedOffset(analysisType);
                areaSeed = settings.permutationSeed + ...
                    analysisSeedOffset + aa;

                [pValuePrimary, jackknifePseudoValues, ...
                    leaveOneParticipantOutBeta, ...
                    nSuccessfulPermutationParticipants] = ...
                    computeParticipantPermutationPValue( ...
                        X, ...
                        T, ...
                        censoring, ...
                        ...
                        settings.minimumSuccessfulParticipantFraction, ...
                        areaSeed, ...
                        coxOptions);
                ...
                areaModels.(safeAreaName) = struct( ...
                    ...
                    'waldPValuePrimary', waldPValuePrimary, ...
                    'permutationPValuePrimary', pValuePrimary, ...
    ...
    %% Multiple-comparison correction across fitted anatomical areas

    fittedRows = areaResults.status == "fitted" & ...
        isfinite(areaResults.pValue);

    if any(fittedRows)
        areaResults.pValueFDR(fittedRows) = ...
            benjaminiHochberg(areaResults.pValue(fittedRows));

        areaResults.significantFDR(fittedRows) = ...
            areaResults.pValueFDR(fittedRows) < 0.05;
    end
...
function seedOffset = getAnalysisSeedOffset(analysisType)
...
function [pValue, pseudoValues, leaveOneOutBeta, nSuccessful] = ...
    computeParticipantPermutationPValue( ...
        X, T, censoring, patientID, observedBetaPrimary, ...
        numberOfPermutations, minimumSuccessfulFraction, ...
        randomSeed, coxOptions)

    patientID = string(patientID(:));
    patientLevels = unique(patientID, 'stable');
    numberOfParticipants = length(patientLevels);
    leaveOneOutBeta = NaN(numberOfParticipants, 1);

    for participantNumber = 1:numberOfParticipants

        keepRows = patientID ~= patientLevels(participantNumber);

        if sum(keepRows) <= size(X, 2)
            continue;
        end

        XLeaveOneOut = X(keepRows, :);
        TLeaveOneOut = T(keepRows, :);
        censoringLeaveOneOut = censoring(keepRows);
        patientIDLeaveOneOut = patientID(keepRows);

        [strataLeaveOneOut, ~] = ...
            findgroups(patientIDLeaveOneOut);

        validRows = ...
            all(isfinite(XLeaveOneOut), 2) & ...
            all(isfinite(TLeaveOneOut), 2) & ...
            TLeaveOneOut(:, 1) >= 0 & ...
            TLeaveOneOut(:, 2) > TLeaveOneOut(:, 1) & ...
            isfinite(strataLeaveOneOut);

        XLeaveOneOut = XLeaveOneOut(validRows, :);
        TLeaveOneOut = TLeaveOneOut(validRows, :);
        censoringLeaveOneOut = ...
            censoringLeaveOneOut(validRows);
        strataLeaveOneOut = strataLeaveOneOut(validRows);

        if isempty(XLeaveOneOut) || ...
                ~any(XLeaveOneOut(:, 1) ~= XLeaveOneOut(1, 1))
            continue;
        end

        varyingColumns = any( ...
            XLeaveOneOut ~= XLeaveOneOut(1, :), 1);
        varyingColumns(1) = true;
        XLeaveOneOut = XLeaveOneOut(:, varyingColumns);

        if size(XLeaveOneOut, 1) <= size(XLeaveOneOut, 2)
            continue;
        end

        try
            betaLeaveOneOut = coxphfit( ...
                XLeaveOneOut, ...
                TLeaveOneOut, ...
                'Censoring', censoringLeaveOneOut, ...
                'Strata', strataLeaveOneOut, ...
                'Ties', 'efron', ...
                'Baseline', 0, ...
                'Options', coxOptions);

            if ~isempty(betaLeaveOneOut) && ...
                    isfinite(betaLeaveOneOut(1))
                leaveOneOutBeta(participantNumber) = ...
                    betaLeaveOneOut(1);
            end
    ...
    successfulRows = isfinite(leaveOneOutBeta);
    nSuccessful = sum(successfulRows);
    minimumSuccessful = max(2, ceil( ...
        minimumSuccessfulFraction * numberOfParticipants));

    if nSuccessful < minimumSuccessful
        error(['Only %d of %d leave-one-participant-out fits ' ...
            'succeeded; at least %d were required.'], ...
            nSuccessful, numberOfParticipants, minimumSuccessful);
    end

    pseudoValues = ...
        numberOfParticipants * observedBetaPrimary - ...
        (numberOfParticipants - 1) .* ...
        leaveOneOutBeta(successfulRows);

    observedStatistic = abs(mean(pseudoValues));
    ...
    randomSigns = 2 .* (rand( ...
        numberOfPermutations, nSuccessful) >= 0.5) - 1;

    randomizedStatistics = abs(mean( ...
        randomSigns .* pseudoValues(:)', 2));

    validRandomizedRows = isfinite(randomizedStatistics);
    numberOfSuccessfulPermutations = sum(validRandomizedRows);

    if numberOfSuccessfulPermutations == 0
        error('No valid randomized statistics were generated.');
    end

    pValue = (1 + sum( ...
        randomizedStatistics(validRandomizedRows) >= ...
        observedStatistic)) / ...
        (numberOfSuccessfulPermutations + 1);
...
function plotAreaForestPanel( ...
    ...
    if isempty(plotData)
        ...
        ax.YLim = [0 1];
        ax.YTick = [];
        text(ax, 0, 0.5, 'No FDR-significant areas', ...
