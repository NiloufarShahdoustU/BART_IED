% Save participant-level significant-channel counts for the expected-reward models.
%
% Run IED8_cox_expected_reward_by_brain_area first. This script then reads
...
outputFile = fullfile(brainAreaResultsFolder, ...
    'significant_expected_reward_channels_by_participant.csv');

barPlotFile = fullfile(brainAreaResultsFolder, ...
    'significant_expected_reward_channels_IT_RT_BR_barplot.pdf');
...
%% Match participants to FDR-significant interaction areas
...
for aa = 1:numel(analysisNames)
    ...
    significantRows = ...
        ...
        isfinite(areaResults.pValueFDR_IEDxExpectedReward);

    significantResults = areaResults(significantRows, :);

    fprintf('\n%s: %d FDR-significant post-IED x expected-reward areas.\n', ...
    ...
    for pp = 1:numel(fileList)
        ...
        for ss = 1:height(significantResults)
            ...
            newRow = table( ...
                ...
                resultRow.pValue_IEDxExpectedReward, ...
                resultRow.pValueFDR_IEDxExpectedReward, ...
                resultRow.interactionHazardRatio, ...
...
if ~isempty(outputTable)
    outputTable = sortrows(outputTable, ...
        {'analysis', 'pValueFDR_IEDxExpectedReward', 'anatomicalArea', 'patientID'}, ...
...
function T = initializeOutputTable()
    T = table( ...
        ...
        'VariableNames', { ...
            ...
            'pValue_IEDxExpectedReward', 'pValueFDR_IEDxExpectedReward', 'interactionHazardRatio', ...
...
function createSignificantAreaBarPlot(T, analysisNames, outputFile)
    ...
    for aa = 1:numel(analysisNames)
        ...
        pValueFDR = splitapply(@(x) x(1), ...
            outcomeTable.pValueFDR_IEDxExpectedReward, areaGroup);
        ...
        xlabel(ax, 'significant expected-reward IED channels', ...
    ...
    title(layout, ...
        'significant post-IED x expected-reward areas', ...
        'FontSize', 16, ...
        'FontWeight', 'bold');
