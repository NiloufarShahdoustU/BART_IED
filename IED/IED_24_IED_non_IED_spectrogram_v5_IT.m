...
outputFolderName = '\\155.100.91.44\d\Code\Nill\BART\IED\IED_24_IED_non_IED_spectrogram_v5_IT\';
...
% for pt = 1:1
    ...
    % also store if an epoch existed for that area
    areaEpochExists = struct();
    ...
    for e = 1:length(epochList)
        ...
        for iA = 1:numel(BrainAreasCommon_epoch)
            ...
            areaEpochExists.(areaSafe).(epochName) = true;
    ...
    for a = 1:numel(areaFields)
        ...
        fig = figure('Color','w','Position',[100 100 1200 380*nEpochPlot], 'Visible','on');
