...
outputFolderName_vecs = '\\155.100.91.44\d\Code\Nill\BART\IED\IED_19_IEDnonIED_bhvr_analysis\v3\RTs\';
...
% for pt = 1:1
    ...
    if sum(IEDChanPercent_PreOnset1~=0) % if any trials with IEDs are left
        NonIEDTrials_bhvr_measure = RTs(IEDChanPercent_PreOnset1 == 0);
        IEDTrials_bhvr_measure = RTs(IEDChanPercent_PreOnset1 ~= 0);
    ...
    if sum(IEDChanPercent_PreOnset2~=0) % if any trials with IEDs are left
        NonIEDTrials_bhvr_measure = RTs(IEDChanPercent_PreOnset2 == 0);
        IEDTrials_bhvr_measure = RTs(IEDChanPercent_PreOnset2 ~= 0);
    ...
    if sum(IEDChanPercent_PostOnset~=0) % if any trials with IEDs are left
        NonIEDTrials_bhvr_measure = RTs(IEDChanPercent_PostOnset == 0);
        IEDTrials_bhvr_measure = RTs(IEDChanPercent_PostOnset ~= 0);
   ...
   if sum(IEDChanPercent_PreResponse~=0) % if any trials with IEDs are left 
        NonIEDTrials_bhvr_measure = RTs(IEDChanPercent_PreResponse == 0);
        IEDTrials_bhvr_measure = RTs(IEDChanPercent_PreResponse ~= 0);
   ...
   if sum(IEDChanPercent_PostResponse~=0) % if any trials with IEDs are left         
        NonIEDTrials_bhvr_measure = RTs(IEDChanPercent_PostResponse == 0);
        IEDTrials_bhvr_measure = RTs(IEDChanPercent_PostResponse ~= 0);
   ...
   if sum(IEDChanPercent_PreOutcome~=0) % if any trials with IEDs are left
        NonIEDTrials_bhvr_measure = RTs(IEDChanPercent_PreOutcome == 0);
        IEDTrials_bhvr_measure = RTs(IEDChanPercent_PreOutcome ~= 0);
...
pValuePreOnset1 = permutationTest(Clean_nonIEDtrials_bhvr_measure_mean_aggregate, ...
                          Clean_IEDTrials_bhvr_measure_mean_PreOnset1, NumberofPermutations);

pValuePreOnset2 = permutationTest(Clean_nonIEDtrials_bhvr_measure_mean_aggregate, ...
                          Clean_IEDTrials_bhvr_measure_mean_PreOnset2, NumberofPermutations);

pValuePostOnset = permutationTest(Clean_nonIEDtrials_bhvr_measure_mean_aggregate, ...
                          Clean_IEDTrials_bhvr_measure_mean_PostOnset, NumberofPermutations);

pValuePreResponse = permutationTest(Clean_nonIEDtrials_bhvr_measure_mean_aggregate, ...
                          Clean_IEDTrials_bhvr_measure_mean_PreResponse, NumberofPermutations);

%%


% Group all vectors into one matrix
all_data = [vec1(:); vec2(:); vec3(:); vec4(:); vec5(:)];
...
group = [ones(size(vec1(:))); 2*ones(size(vec2(:))); 3*ones(size(vec3(:))); ...
         4*ones(size(vec4(:))); 5*ones(size(vec5(:)))];
...
filePath = fullfile(outputFolderName, 'RTs_stat.txt');
