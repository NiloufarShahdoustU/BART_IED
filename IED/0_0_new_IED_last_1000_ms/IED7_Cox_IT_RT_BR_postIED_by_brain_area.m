...
% Forest-plot labels report, for every displayed area:
%   1) participants with at least one valid IED in that area, and
%   2) participants with electrode coverage in that area.
...
outputFolderName = ...
    'D:\Nill\code\BART\IED\0_0_new_IED_last_1000_ms\IED7_Cox_IT_RT_BR_postIED_by_brain_area\';
...
function [areaResults, areaModels] = runLoggedBrainAreaAnalysis( ...
    ...
    fprintf('Uncorrected significant areas (p < 0.05): %d\n', ...
...
function [areaResults, areaModels] = runBrainAreaAnalysis( ...
    ...
    %% Uncorrected significance across fitted anatomical areas
...
function plotAreaForestPanel( ...
    ...
    if isempty(plotData)
        ...
        ax.YLim = [0 1];
        ax.YTick = [];
        text(ax, 0, 0.5, 'No areas with uncorrected p < 0.05', ...
