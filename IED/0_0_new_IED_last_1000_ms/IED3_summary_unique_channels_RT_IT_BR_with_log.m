...
inputFolderName_LFPIED = 'D:\Nill\data\BART\0_0_new_IED_last_1000_ms\IED1_find_number_of_IEDs\';
outputFolderName = 'D:\Nill\code\BART\IED\0_0_new_IED_last_1000_ms\IED3_summary_unique_channels_RT_IT_BR_with_log\';
...
for pt = 1:length(fileList)
    ...
    for pp = 1:size(panels, 1)
        ...
        yAll = panels{pp, 5};
        keepIdx = panels{pp, 6};
        durationAll = panels{pp, 7};

        rawX = xAll(keepIdx);
        y = yAll(keepIdx);
        duration = durationAll(keepIdx);

        [rawX, logX, y, duration] = cleanPanelData(rawX, y, yMeasure, duration);

        statsRow = summarizeOnePanel(rawX, logX, y, yMeasure, duration);
        ...
        if string(yMeasure) == "BR"
            ...
            newPRRow.modelIntercept = -statsRow.modelIntercept;
            newPRRow.modelSlope = -statsRow.modelSlope;
            newPRRow.modelSlopeSE = statsRow.modelSlopeSE;
            newPRRow.modelSlopeT = -statsRow.modelSlopeT;
            newPRRow.modelSlopeP = statsRow.modelSlopeP;
            newPRRow.modelSlopeCILow = -statsRow.modelSlopeCIHigh;
            newPRRow.modelSlopeCIHigh = -statsRow.modelSlopeCILow;
            newPRRow.oddsRatio = exp(-statsRow.modelSlope);
            newPRRow.oddsRatioCILow = exp(-statsRow.modelSlopeCIHigh);
            newPRRow.oddsRatioCIHigh = exp(-statsRow.modelSlopeCILow);
            newPRRow.modelType = "Per-patient logistic GLM: PR = 1 - BR, adjusted for IT";

            perPatientResults = [perPatientResults; newPRRow];
...
function [rawX, logX, y, duration] = cleanPanelData(rawX, y, yMeasure, duration)
    ...
    if string(yMeasure) == "BR"
        validIdx = isfinite(rawX) & isfinite(y) & isfinite(duration) & ...
            rawX >= 0 & duration > 0 & (y == 0 | y == 1);
    else
...
function statsRow = summarizeOnePanel(rawX, logX, y, yMeasure, duration)
    ...
    if string(yMeasure) == "BR"
        ...
        try
            ...
            T = table(logX, duration, y, ...
                'VariableNames', {'logX', 'duration', 'y'});

            glm = fitglm(T, 'y ~ logX + duration', ...
            ...
            statsRow.modelType = "Per-patient logistic GLM: BR ~ logX + IT";
...
function groupSlopeSummary = makeGroupSlopeSummaryFromPerPatient(perPatientResults, nPermutations)
    ...
    for cc = 1:length(comparisonNames)
        ...
        if nPatientsWithSlope > 0
            ...
            if rows.yMeasure(1) == "BR" || rows.yMeasure(1) == "PR"
    ...
    % correct across rt, it, and br only, then copy br correction to pr
    uniqueP = groupSlopeSummary.permutationP_Slope(1:3);
    [uniqueBonf, uniqueFDR] = multipleHypothesisCorrection(uniqueP);

    pBonf = NaN(height(groupSlopeSummary), 1);
    pFDR = NaN(height(groupSlopeSummary), 1);
    pBonf(1:3) = uniqueBonf;
    pFDR(1:3) = uniqueFDR;
    pBonf(4) = pBonf(3);
    pFDR(4) = pFDR(3);
...
function groupMixedEffectsResults = runGroupMixedEffects(trialLevelData)
    ...
    for cc = 1:length(comparisonNames)
        ...
        if isBR
            rows = rowsAll(isfinite(rowsAll.logX) & isfinite(rowsAll.y) & ...
                isfinite(rowsAll.duration) & rowsAll.duration > 0, :);
        else
            rows = rowsAll(isfinite(rowsAll.logX) & isfinite(rowsAll.y), :);
        end
            ...
            if isBR
                ...
                if length(unique(rows.y)) <= 1
                    ...
                    try
                        ...
                        glme = fitglme(rows, 'y ~ logX + duration + (1 | patientID)', ...
                            'Distribution', 'Binomial', ...
                            'Link', 'logit');
                        ...
                        glm = fitglm(rows, 'y ~ logX + duration', ...
                            'Distribution', 'binomial', ...
                            'Link', 'logit');
                        ...
                        modelType = "Pooled logistic GLM fallback adjusted for IT";
        ...
        % report the same binary result with pop coded as the event
        if isBR
    ...
    uniqueP = groupMixedEffectsResults.pValue(1:3);
    [uniqueBonf, uniqueFDR] = multipleHypothesisCorrection(uniqueP);

    pBonf = NaN(height(groupMixedEffectsResults), 1);
    pFDR = NaN(height(groupMixedEffectsResults), 1);
    pBonf(1:3) = uniqueBonf;
    pFDR(1:3) = uniqueFDR;
    pBonf(4) = pBonf(3);
    pFDR(4) = pFDR(3);
...
function labels = makePrettyComparisonLabels(comparisonOrder)
    ...
    for ii = 1:length(comparisonOrder)
        ...
        elseif contains(comparisonName, "BR")
            labels(ii) = "BR";
        elseif contains(comparisonName, "PR")
            labels(ii) = "PR";
        else
