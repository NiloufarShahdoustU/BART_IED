...
% for pt = 1:1
    ...
    for trial = 1:nTrials
        ...
        IEDChanPercent_PreOnset1(trial) = sum(IEDtrialsPreOnset1_only(:,trial))/nChans;  % percent of channels with IED occuring in them
        IEDChanPercent_PreOnset2(trial) = sum(IEDtrialsPreOnset2_only(:,trial))/nChans;
        IEDChanPercent_PostOnset(trial) = sum(IEDtrialsPostOnset_only(:,trial))/nChans;
        IEDChanPercent_PreResponse(trial) = sum(IEDtrialsPreResponse_only(:,trial))/nChans;
        IEDChanPercent_PostResponse(trial) = sum(IEDtrialsPostResponse_only(:,trial))/nChans;
        IEDChanPercent_PreOutcome(trial) = sum(IEDtrialsPreOutcome_only(:,trial))/nChans;
                      
    end % end for trial

    % we are ONLY taking into account IED trials that IEDs occur in more
    % than 5% of channels:
    IEDpercentThreshold = 0.01;
...
nonIEDtrials_bhvr_measure_mean_aggregate = nansum(nonIEDtrials_bhvr_measure_mean,2);
