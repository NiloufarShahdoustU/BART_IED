...
% each epoch.
% UPDATED: just like V4 but only for accuracy!!
% UPDATED2: NEW VIS: ONE FIGURE PER PARTICIPANT
%   2 rows (epochs) x N columns (brain areas)
%   each subplot: Power(dB) vs Frequency(Hz) with 3 curves (IED sig / IED non-sig / non-IED)
%   mean = solid line, SEM = shaded (alpha=0.4)
% UPDATED3:
%   x ticks start at 0, but x-axis has a small left pad (so curve isn't glued to y-axis)
%   each subplot square
% AUTHOR: Nill (edited)
...
outputFolderName = '\\155.100.91.44\d\Code\Nill\BART\IED\IED_25_IED_non_IED_freq_power_vis_v1_acc\';
...
% for pt = 1:1
    ...
    areaData = struct(); % areaSafe -> .areaName + per epoch fields
    ...
    % figure size (try to help square tiles)
    tilePx = 300; % approx square tile size in pixels
    figW = max(1200, tilePx*nAreas);
    figH = max(700,  tilePx*2 + 120);
    fig = figure('Color','w','Position',[60 60 figW figH], 'Visible','off');

    tl = tiledlayout(2, nAreas, 'Padding','compact', 'TileSpacing','compact');
        ...
        for a = 1:nAreas
            ...
            if isfield(areaData.(areaSafe), epochName)
                ...
                if ~legSet
                    ...
                    htmp = flipud(htmp(:)); % order of creation
                    if numel(htmp) >= 3
    ...
    if legSet
        ...
        % make legend text colors match the curve colors
        lg.TextColor = 'black'; % just to be explicit (we will override per entry)
        legTxt = lg.String; 
    
        % IMPORTANT: in many MATLAB versions, legend text objects are returned by:
        txtObjs = findobj(lg, 'Type', 'Text');
        % They often come back in reverse order, so flip:
...
% convert to dB, then compute mean+SEM across samples (per frequency).
    ...
    for ii = 1:numel(chanIdxPerArea)
        ...
        for t = 1:size(SpecCell,2)
            M = SpecCell{r,t};
            if ~isempty(M)
                v_lin = mean(M, 2, 'omitnan');    % (freq x 1)
                v_dB  = 10*log10(v_lin + eps);    % (freq x 1)
