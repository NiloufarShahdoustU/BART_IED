% ccn 2026
% IT: power box plot (BOXPLOT + SCATTER per trial, band 8-80 Hz)
...
outputFolderName = '\\155.100.91.44\d\Code\Nill\BART\IED\IED_26_IED_non_IED_power_boxplot_v1_IT\';
...
% for pt = 1:1
    ...
    % ---- store per-area, per-epoch trial points (dB band power) ----
    areaData = struct(); % areaSafe -> .areaName + per epoch: pts_sig/pts_nonsig/pts_nonIED
    ...
    sgtitle(sprintf('%s | IT Band power boxplots', ptID), 'Interpreter','none');

    % *** NEW FILE NAME so you can’t confuse it with curves ***
    fnameBase = sprintf('%s_IT_BoxplotBandPower_%dto%dHz_2epochs_ALLareas_yTop%d_stats', ...
