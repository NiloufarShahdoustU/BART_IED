...
%
% UPDATED (ALL PATIENTS IN ONE FIGURE, BASELINE-CORRECTED):
% - one plot for ALL patients (no per-patient figures)
% - each scatter dot = one patient's mean (per condition)
% - baseline issue handled by within-patient delta from non-IED:
%       Δsig    = m_sig    - m_nonIED
%       Δnonsig = m_nonsig - m_nonIED
% - plot 2 boxplots: Δsig and Δnonsig (non-IED baseline is 0 but not plotted)
% - tests per subplot (paired across patients):
%       (1) Δsig vs 0        (sig vs nonIED)
%       (2) Δnonsig vs 0     (nonsig vs nonIED)
%       (3) Δsig vs Δnonsig  (sig vs nonsig)
%   then Holm correction within subplot (3 tests)
% - annotate each box with n.s./stars vs 0, and bar between boxes with n.s./stars
%
...
outputFolderName = '\\155.100.91.44\d\Code\Nill\BART\IED\IED_26_IED_non_IED_power_boxplot_v1_acc_all_patients\';
...
areaDataAll = struct(); % store ptIdx + per-condition patient means
f_Hz_ref = [];
...
% for pt = 1:5
    ...
    for e = 1:length(epochList)
        ...
        for iA = 1:numel(BrainAreasCommon_epoch)
            ...
            areaDataAll.(areaSafe).(epochName).ptIdx(end+1,1)       = pt;       
            areaDataAll.(areaSafe).(epochName).sigMeans(end+1,1)    = m_sig;    
            areaDataAll.(areaSafe).(epochName).nonsigMeans(end+1,1) = m_nonsig; 
            areaDataAll.(areaSafe).(epochName).nonIEDMeans(end+1,1) = m_nonIED; 
...
% NOTE: This version forces y-axis limits to [-30, 30] on every subplot.
...
for e = 1:numel(epochNames)
    ...
    for a = 1:nAreas
        ...
        if isfield(areaDataAll.(areaSafe), epochName)
            ...
            % ---- compute within-patient deltas (paired across patients) ----
            ...
            % scatters
            jitter = 0.18;
            x1 = 1 + (rand(size(dSig))-0.5)*2*jitter;
            x2 = 2 + (rand(size(dNonSig))-0.5)*2*jitter;

            scatter(ax, x1, dSig,    12, 'filled', 'MarkerFaceAlpha', 0.5, 'MarkerEdgeAlpha', 0.0);
            scatter(ax, x2, dNonSig, 12, 'filled', 'MarkerFaceAlpha', 0.5, 'MarkerEdgeAlpha', 0.0);
            ...
            % ---- FIXED y-lims ----
            ylim(ax, [yLowFixed, yTopFixed]);

            % ---- tests (paired) ----
            % 1) dSig vs 0  (sig vs nonIED)
            % 2) dNonSig vs 0 (nonsig vs nonIED)
            % 3) dSig vs dNonSig (sig vs nonsig)
            ...
            if okStats
                try
                    pRaw(1) = signrank(dSig, 0);
                    pRaw(2) = signrank(dNonSig, 0);
                    pRaw(3) = signrank(dSig, dNonSig);
                    ok = isfinite(pRaw);
                catch
                    ok = false(3,1);
                end
            ...
            % ---- annotate: stars on top of each box for vs nonIED ----
            yl = ylim(ax);
            ySpan = yl(2) - yl(1);
            if ~isfinite(ySpan) || ySpan<=0, ySpan = 1; end

            yTop1 = local_box_top_y(dSig);
            yTop2 = local_box_top_y(dNonSig);

            yTextPad = 0.03 * ySpan;
            if ~isfinite(yTextPad) || yTextPad<=0, yTextPad = 1; end

            % keep label positions inside the fixed axis range
            yLab1 = min(yTop1 + yTextPad, yl(2) - 0.04*ySpan);
            yLab2 = min(yTop2 + yTextPad, yl(2) - 0.04*ySpan);

            lab1 = 'n/a';
            lab2 = 'n/a';
            if ok(1) && isfinite(pRaw(1)), lab1 = local_p_to_stars_or_ns(pRaw(1)); end
            if ok(2) && isfinite(pRaw(2)), lab2 = local_p_to_stars_or_ns(pRaw(2)); end

            text(ax, 1, yLab1, lab1, 'HorizontalAlignment','center', ...
                'VerticalAlignment','bottom', 'Color','k', 'FontSize',10);
            text(ax, 2, yLab2, lab2, 'HorizontalAlignment','center', ...
                'VerticalAlignment','bottom', 'Color','k', 'FontSize',10);

            % ---- significance bar between the two boxes (sig vs nonsig) ----
            yBar = max([yTop1, yTop2]) + 0.10*ySpan;
            if ~isfinite(yBar), yBar = yl(2) - 0.10*ySpan; end
            if yBar > yl(2) - 0.02*ySpan
                yBar = yl(2) - 0.08*ySpan;
            end
            if yBar < yl(1) + 0.10*ySpan
                yBar = yl(1) + 0.15*ySpan;
            end

            labBar = 'n/a';
            if ok(3) && isfinite(pRaw(3)), labBar = local_p_to_stars_or_ns(pRaw(3)); end
        ...
        if a == 1
            ylabel(ax, sprintf('%s\nPatient-mean band power \\Delta from non-IED\n(%d–%d Hz) [dB]', ...
...
sgtitle(sprintf('ALL PATIENTS | Band power (baseline-corrected within patient)'), 'Interpreter','none');

fnameBase = sprintf('ALLPATIENTS_BoxplotBandPower_DELTAfromNonIED_%dto%dHz_2epochs_ALLareas_yLim_%dto%d_signrank_RAW', ...
    bandLo, bandHi, round(yLowFixed), round(yTopFixed));
...
function [dSig, dNonSig] = local_within_patient_deltas(ptIdx, mSig, mNonSig, mNonIED)
    % Returns paired deltas per patient:
    %   dSig    = mSig - mNonIED
    %   dNonSig = mNonSig - mNonIED
    % Keeps only patients with finite values for all.
    ptIdx = ptIdx(:);
...
function yTop = local_box_top_y(x)
    % robust-ish top for placing labels (use 75th percentile + 1.5*IQR like whisker-ish)
    x = x(isfinite(x));
    if isempty(x)
        yTop = 0;
        return;
