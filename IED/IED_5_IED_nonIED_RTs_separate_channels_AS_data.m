...
outputFolderName = '\\155.100.91.44\d\Data\Nill\BART\IEDdata_RTs\'; 
Fnew = 500;
...
% for pt = 1:1
    ...
    for row = 1:nChans-1
        for col = 1:nTrials
            ...
            if IEDtrials(row, col) == 0 
                NonIEDTrialsRTs(row, col) = ReactionTimesFiltered(col);
            end
            ...
            if any(slice(:) == 1)
                IEDtrials_preStimRT(row, col) = ReactionTimesFiltered(col);
            end
            ...
            slice(isnan(slice)) = 0; % Replace NaN with 0
            if any(slice(:) == 1)
                 IEDtrials_periStimRT(row, col) = ReactionTimesFiltered(col);
            end
            ...
            slice(isnan(slice)) = 0; % Replace NaN with 0
            if any(slice(:) == 1)
                 IEDtrials_postStimRT(row, col) = ReactionTimesFiltered(col);
            end
    ...
    IEDdata.pVal_periStimRTfiltered = pVal_periStimRT_filtered;
