...
clc;
close all;
%% loading new labels from Brainnetome instead of MNI

Brainnetome_data_folder = 'D:\Nill\code\BART\IED\0_0_new_IED_new_area_labels\IED0_VR_brainRegions';
matFiles = dir(fullfile(Brainnetome_data_folder, '*.mat'));
...
outputFolderName = 'D:\Nill\data\BART\0_0_new_IED_new_area_labels\IED1_find_number_of_IEDs\'; 
if ~exist(outputFolderName, 'dir')
    mkdir(outputFolderName);
end
%%
 
for pt = 16:length(fileList)
% for pt = 16:length(fileList)
% for pt = 1:1
    ...
    % finding non empty and non unknown labels in the Brainnetome_Atlas_All_woHemi
    selectedChans = find(~cellfun(@isempty, Brainnetome_Atlas_All_woHemi(:, pt)) & ...
                     ~strcmp(Brainnetome_Atlas_All_woHemi(:, pt), 'Unk'));
    

   
    nChans = length(selectedChans);
    anatomicalLocs = Brainnetome_Atlas_All_woHemi(selectedChans, pt);
    anatomicalLocs_wHemisphere = Brainnetome_Atlas_All_wHemi(selectedChans, pt);
    ...
    LFPIED.anatomicalLocs_wHemisphere = anatomicalLocs_wHemisphere;
    ...
    clear ch2 tt LFPmat LFPmat_temp_vec ch data2K balloonTimes balloonType anatomicalLocs nTrials anatomicalLocs_wHemisphere
