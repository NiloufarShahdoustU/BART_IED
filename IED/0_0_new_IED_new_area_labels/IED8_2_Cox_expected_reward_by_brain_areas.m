...
% Within-trial IED-time permutation test
...
function [areaResults, areaModels] = runLoggedMechanisticAnalysis( ...
    ...
    fprintf([ ...
        'Permutation-significant IED x expected-reward interactions ' ...
        '(uncorrected p < 0.05): %d\n'], ...
        sum(areaResults.significantPermutation_IEDxExpectedReward));
    fprintf('Saved: %s\n', resultsOutputFile);
    fprintf('Saved: %s\n', modelOutputFile);
    fprintf('Saved: %s\n', textOutputFile);
...
function [areaResults, areaModels] = runMechanisticBrainAreaAnalysis( ...
    ...
    for pt = 1:length(fileList)
        ...
        % Match the modeling file by participant ID. Matching files by their
        % positions in two separate dir listings can silently pair data from
        % different participants when the directory sort orders differ.
    ...
    for aa = 1:length(allAreas)
        ...
        interactionPermutationP = NaN;
        nSuccessfulPermutations = 0;
        primaryInteractionP = NaN;
        safeAreaName = "";
        ...
        if status == "fitted" && isfinite(interactionSummary.z)
            [interactionPermutationP, nSuccessfulPermutations] = ...
                runMechanisticInteractionPermutationTest( ...
                    participants, anatomicalArea, ...
                    postIEDWindowSeconds, interactionSummary.z, settings);
            ...
            if nSuccessfulPermutations >= minimumSuccessfulPermutations
                primaryInteractionP = interactionPermutationP;
            else
        ...
        if strlength(safeAreaName) > 0 && ...
            ...
            areaModels.(char(safeAreaName)).pValueWaldInteraction = ...
                interactionSummary.p;
            areaModels.(char(safeAreaName)).pValuePermutationInteraction = ...
                interactionPermutationP;
        ...
        if status == "fitted"
            ...
            fprintf([ ...
                ...
                interactionSummary.p, interactionPermutationP, ...
    ...
    %% Uncorrected permutation significance for the PRIMARY interaction

    fittedRows = areaResults.status == "fitted" & ...
        isfinite(areaResults.pValue_IEDxExpectedReward);

    if any(fittedRows)
        areaResults.significantPermutation_IEDxExpectedReward( ...
            fittedRows) = ...
...
function results = initializeMechanisticResultsTable()
    ...
    results = table( ...
        ...
        zeros(0, 1), ...    % pValueWald_IEDxExpectedReward
        zeros(0, 1), ...    % pValuePermutation_IEDxExpectedReward
        zeros(0, 1), ...    % nSuccessfulPermutations
        zeros(0, 1), ...    % pValue_IEDxExpectedReward (primary)
        ...
        false(0, 1), ...    % significantPermutation_IEDxExpectedReward
        'VariableNames', { ...
            'anatomicalArea', ...
            'postIEDWindowMilliseconds', ...
            'status', ...
            ...
            'pValueWald_IEDxExpectedReward', ...
            'pValuePermutation_IEDxExpectedReward', ...
            'nSuccessfulPermutations', ...
            ...
            'significantPermutation_IEDxExpectedReward'});
...
function modelFileName = findParticipantModelFile( ...
    ...
    % Some datasets concatenate the ID directly with the file suffix. Use
    % contains only as a fallback and only when it gives one unique match.
...
function [permutationPValue, nSuccessfulPermutations] = ...
    runMechanisticInteractionPermutationTest( ...
        ...
        observedInteractionRobustZ, settings)

    nPermutations = settings.nPermutations;
    permutationRobustZ = NaN(nPermutations, 1);
    ...
    for permutationNumber = 1:nPermutations
        permutedData = emptyMechanisticCountingProcessTable();
        ...
        permutationRobustZ(permutationNumber) = ...
            fitMechanisticInteractionRobustZ( ...
                permutedData, settings);
    ...
    validPermutationRobustZ = permutationRobustZ( ...
        isfinite(permutationRobustZ));
    nSuccessfulPermutations = length(validPermutationRobustZ);

    if nSuccessfulPermutations == 0 || ...
            ~isfinite(observedInteractionRobustZ)
        permutationPValue = NaN;
        return;
    end

    permutationPValue = (1 + sum( ...
        abs(validPermutationRobustZ) >= ...
        abs(observedInteractionRobustZ))) ./ ...
...
function interactionRobustZ = fitMechanisticInteractionRobustZ( ...
    countingProcessData, settings)

    interactionRobustZ = NaN;
    ...
    try
        ...
        [~, ~, robustZ, ~] = computeClusterRobustInference( ...
            stats, beta, countingProcessData.patientStratum);

        if length(robustZ) >= interactionIndex && ...
                isfinite(robustZ(interactionIndex))
            interactionRobustZ = robustZ(interactionIndex);
        end
    catch
        interactionRobustZ = NaN;
    end
...
function plotInteractionForestPanel( ...
    ...
    if isempty(plotData)
        ...
        ax.YTick = [];
        text(ax, 0, 0.5, ...
            'No uncorrected permutation-significant interactions', ...
        ...
        for ii = 1:nAreas
            areaTickLabels(ii) = sprintf( ...
                '%s  |  IED participants: %d  |  covered: %d', ...
    ...
    title(ax, {char(descriptiveTitle), windowLabel, ...
        'Permutation p < 0.05 (uncorrected)'}, ...
        'FontSize', 12, ...
        'FontWeight', 'bold');
