...
function plotHemisphereForestPanel(ax, result, config, ...
    ...
    xlabel(ax, 'adjusted hazard ratio with robust 95% CI');
    title(ax, sprintf('%s: hemisphere-specific IED effects', config.code), ...
        'FontWeight', 'bold');
...
function plotHemisphereContrastPanel(ax, result, config)
    ...
    xlabel(ax, 'hazard-ratio ratio with robust 95% CI');
    title(ax, sprintf('%s: direct left-right contrast', config.code), ...
        'FontWeight', 'bold');
