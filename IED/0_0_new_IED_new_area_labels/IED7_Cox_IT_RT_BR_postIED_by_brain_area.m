function IED7_Cox_IT_RT_BR_postIED_by_brain_area

% Combined area-specific participant-stratified Cox analyses
% BR is one stacked competing-transition model with separate coefficients
% for banking and popping.
% BH-FDR correction is performed within each anatomical area across
% exactly four effects: IT, RT, BR (bank), and PR (pop).
% Permutation p-values use 1000 within-participant, within-color trial
% shuffles of RT, IT, and the bank/pop outcome only.
% The final corrected IT, RT, and BR/PR CSV files and forest-plot PDFs
% are saved. No MAT files, raw CSV files, or console logs are saved.
...
outputFolderName = ...
    'D:\Nill\code\BART\IED\0_0_new_IED_new_area_labels\IED7_Cox_IT_RT_BR_postIED_by_brain_area\';
...
% Trial-level permutation settings
...
fprintf('Permutation test: %d trial-level shuffles.\n', ...
...
function [areaResults, areaModels] = runBrainAreaAnalysis( ...
    ...
    for aa = 1:length(allAreas)
        ...
        if status == "fitted"
            fprintf('Running %d trial-level permutations for %s...\n', ...
            ...
            [permutationPValuePrimary, ...
             ...
             nValidPermutationsBankVsPop] = ...
                runAreaPermutationTest( ...
                    ...
                    betaPrimary, betaPop, betaBankMinusPop);
        end
...
function [permutationPValuePrimary, permutationPValuePop, ...
    ...
    nValidBankVsPop] = runAreaPermutationTest( ...
        participants, anatomicalArea, analysisType, ...
        postIEDWindowSeconds, settings, observedBetaPrimary, ...
        observedBetaPop, observedBetaBankMinusPop)

    numberOfPermutations = settings.numberOfPermutations;
    minimumValidPermutations = settings.minimumValidPermutations;

    permutedBetaPrimary = NaN(numberOfPermutations, 1);
    permutedBetaPop = NaN(numberOfPermutations, 1);
    permutedBetaBankMinusPop = NaN(numberOfPermutations, 1);
    ...
    for permutationNumber = 1:numberOfPermutations
        ...
        try
            ...
            if ~isempty(betaPermutation) && ...
                    isfinite(betaPermutation(1))
                permutedBetaPrimary(permutationNumber) = ...
                    betaPermutation(1);
            end

            if analysisType == "BR" && ...
                    length(betaPermutation) >= 2 && ...
                    isfinite(betaPermutation(2))
                permutedBetaPop(permutationNumber) = ...
                    betaPermutation(2);
                permutedBetaBankMinusPop(permutationNumber) = ...
    ...
    validPrimary = isfinite(permutedBetaPrimary);
    validPop = isfinite(permutedBetaPop);
    validBankVsPop = isfinite(permutedBetaBankMinusPop);
    ...
    if isfinite(observedBetaPrimary) && ...
            nValidPrimary >= minimumValidPermutations
        permutationPValuePrimary = ...
            (1 + sum(abs(permutedBetaPrimary(validPrimary)) >= ...
                abs(observedBetaPrimary))) / ...
            (nValidPrimary + 1);
    end

    if analysisType == "BR"
        if isfinite(observedBetaPop) && ...
                nValidPop >= minimumValidPermutations
            permutationPValuePop = ...
                (1 + sum(abs(permutedBetaPop(validPop)) >= ...
                    abs(observedBetaPop))) / ...
                (nValidPop + 1);
        end

        if isfinite(observedBetaBankMinusPop) && ...
                nValidBankVsPop >= minimumValidPermutations
            permutationPValueBankVsPop = ...
                (1 + sum(abs( ...
                    permutedBetaBankMinusPop(validBankVsPop)) >= ...
                    abs(observedBetaBankMinusPop))) / ...
                (nValidBankVsPop + 1);
        end
    end
...
function permutedData = buildPermutedAreaCountingProcessData( ...
    ...
    for pt = 1:length(participants)
        ...
        shuffledDurationSeconds = P.durationSeconds;
        shuffledBankedTrials = P.BankedTrials;

        % Shuffle behavioral values only. IED rows, channels, trial IDs,
        % balloon colors, and participant labels remain unchanged.
        for colorCode = 1:3
            colorTrials = validTrialNumbers( ...
                P.balloonColorCode(validTrialNumbers) == colorCode);

            if length(colorTrials) < 2
                continue;
            end

            sourceTrials = colorTrials(randperm(length(colorTrials)));

            % For RT, durationSeconds contains RT. For IT and BR, it
            % contains IT. In BR, IT and bank/pop outcome move together.
            shuffledDurationSeconds(colorTrials) = ...
                P.durationSeconds(sourceTrials);

            if analysisType == "BR"
                shuffledBankedTrials(colorTrials) = ...
                    P.BankedTrials(sourceTrials);
            end
        end
        ...
        for tt = 1:length(validTrialNumbers)
            ...
            trialNumber = validTrialNumbers(tt);
            durationSeconds = ...
                shuffledDurationSeconds(trialNumber);

            if analysisType == "BR"
                bankObserved = ...
                    shuffledBankedTrials(trialNumber) == 1;
                popObserved = ...
                    shuffledBankedTrials(trialNumber) == 0;
