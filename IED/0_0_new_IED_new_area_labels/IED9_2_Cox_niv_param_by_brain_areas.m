...
% Permutations circularly shift IED times within each original trial.
...
settings.minimumSuccessfulPermutations = 800;
...
for analysisType = analysisTypes
    ...
    [channelResults,channelData] = runChannelAnalysis( ...
        inputFolderName,analysisType,settings,nivTable);
    areaResults = makeAreaNivResults( ...
        channelResults,channelData,analysisType,settings);
...
function [results,channelData] = runChannelAnalysis( ...
    inputFolder,analysisType,S,nivTable)
...
channelData = struct( ...
    'patientID',{},'anatomicalArea',{},'nivAsymmetryRaw',{}, ...
    'nivAsymmetryZ',{},'validTrials',{},'durationSeconds',{}, ...
    'endpointObserved',{},'balloonColorCode',{}, ...
    'channelIEDoccurrence',{},'samplingFrequencyHz',{}, ...
...
for ff = 1:numel(files)
    ...
    for cc = 1:numel(selectedChans)
        ...
        if status=="fitted"
            newChannel.patientID = patientID;
            newChannel.anatomicalArea = labels(cc);
            newChannel.nivAsymmetryRaw = nivRaw;
            newChannel.nivAsymmetryZ = nivZ;
            newChannel.validTrials = valid;
            newChannel.durationSeconds = duration;
            newChannel.endpointObserved = endpoint;
            newChannel.balloonColorCode = balloon;
            newChannel.channelIEDoccurrence = channelIED;
            newChannel.samplingFrequencyHz = Fs;
            newChannel.postIEDWindowSeconds = window;
            channelData(end+1) = newChannel; 
        end
    end
end
...
function A = makeAreaNivResults(R,C,analysisType,S)
...
A = table(repmat(analysisType,n,1),areas,zeros(n,1),zeros(n,1), ...
    ...
    NaN(n,1),zeros(n,1),NaN(n,1),false(n,1),strings(n,1), ...
    'VariableNames',{'analysisType','anatomicalArea','nParticipants', ...
    'nChannels','beta_IEDxNiv','standardError_IEDxNiv', ...
    'betaCILow','betaCIHigh','correlation','tStatistic_IEDxNiv', ...
    'pValue_IEDxNiv','nSuccessfulPermutations', ...
...
for aa = 1:n
    ...
    if ~isfinite(observedT)
        A.status(aa) = "failed: invalid observed slope statistic";
        continue;
    end
    ...
    if isempty(C)
        ...
        areaChannelData = C(string({C.anatomicalArea})'==areas(aa));
    end

    fprintf(['  [%s] area %d/%d: %s; running %d within-trial ' ...
        'circular IED-time permutations'], ...
        analysisType,aa,n,areas(aa),S.numberOfPermutations);
    nullT = NaN(S.numberOfPermutations,1);
    for permutationIndex = 1:S.numberOfPermutations
        nullT(permutationIndex) = permutedAreaSlopeT( ...
            areaChannelData,S);
        if mod(permutationIndex,50)==0 || ...
                permutationIndex==S.numberOfPermutations
            fprintf(' ... %d',permutationIndex);
        end
    end
    fprintf('\n');

    validNull = isfinite(nullT);
    nSuccessful = sum(validNull);
    if nSuccessful>=S.minimumSuccessfulPermutations
        p = (1+sum(abs(nullT(validNull))>=abs(observedT))) / ...
            (nSuccessful+1);
        status = "fitted";
    else
        p = NaN;
    ...
    A.correlation(aa) = r;
    A.tStatistic_IEDxNiv(aa) = observedT;
    A.pValue_IEDxNiv(aa) = p;
    A.nSuccessfulPermutations(aa) = nSuccessful;
    A.status(aa) = status;
end
...
function slopeT = permutedAreaSlopeT(C,S)
slopeT = NaN;
if isempty(C), return; end

nChannels = numel(C);
permutedBeta = NaN(nChannels,1);
patientID = strings(nChannels,1);
nivZ = NaN(nChannels,1);
trialShiftMap = containers.Map('KeyType','char','ValueType','double');

for cc = 1:nChannels
    D = C(cc);
    patientID(cc) = D.patientID;
    nivZ(cc) = D.nivAsymmetryZ;
    trials = find(D.validTrials);
    if isempty(trials), continue; end

    rows = cell(numel(trials),1);
    for tt = 1:numel(trials)
        tr = trials(tt);
        maximumValidSample = ceil( ...
            D.durationSeconds(tr)*D.samplingFrequencyHz)-1;
        shiftKey = sprintf('%s|%d',char(D.patientID),tr);
        if maximumValidSample>=1 && ~isKey(trialShiftMap,shiftKey)
            trialShiftMap(shiftKey) = randi(maximumValidSample)-1;
        end
        if maximumValidSample>=1
            shiftInSamples = trialShiftMap(shiftKey);
        else
            shiftInSamples = 0;
        end
        shiftedIED = circularShiftIEDsWithinTrial( ...
            D.channelIEDoccurrence,tr,D.durationSeconds(tr), ...
            D.samplingFrequencyHz,shiftInSamples);
        rows{tt} = makeRows( ...
            tr,D.durationSeconds(tr),D.endpointObserved(tr), ...
            D.balloonColorCode(tr),shiftedIED, ...
            D.samplingFrequencyHz,D.postIEDWindowSeconds);
    end
    ...
    nOut = sum(CP.eventAtStop & CP.postIED==0);
    if nIn<S.minimumEventsInsideWindow || ...
            nOut<S.minimumEventsOutsideWindow
        continue;
    end

    try
        beta = fitChannelCox(CP,S.minimumEventsInsideWindow);
        if isfinite(beta), permutedBeta(cc) = beta; end
    catch
        % Failed permutation fits remain NaN and are not counted.
    end
end

validChannels = isfinite(permutedBeta) & isfinite(nivZ) & ...
    ~ismissing(patientID);
if ~any(validChannels), return; end

[G,participant] = findgroups(patientID(validChannels));
participantBeta = splitapply(@median,permutedBeta(validChannels),G);
participantNiv = splitapply(@(x)x(1),nivZ(validChannels),G);

ok = ~ismissing(participant) & isfinite(participantBeta) & ...
    isfinite(participantNiv);
participantBeta = participantBeta(ok);
participantNiv = participantNiv(ok);

if numel(participantBeta)<S.minimumParticipantsPerArea || ...
        std(participantBeta)<=sqrt(eps)* ...
            max(1,abs(mean(participantBeta))) || ...
        std(participantNiv)<=sqrt(eps)* ...
            max(1,abs(mean(participantNiv)))
    return;
end

[slope,se] = simpleSlope(participantNiv,participantBeta);
if isfinite(slope) && isfinite(se) && se>0
    slopeT = slope/se;
end
end
...
function shiftedTrialIEDoccurrence = ...
    circularShiftIEDsWithinTrial( ...
        channelIEDoccurrence,trialNumber,durationSeconds, ...
        samplingFrequencyHz,shiftInSamples)

if isempty(channelIEDoccurrence) || ...
        size(channelIEDoccurrence,2)<3 || ...
        ~isfinite(durationSeconds) || durationSeconds<=0 || ...
        ~isfinite(samplingFrequencyHz) || samplingFrequencyHz<=0
    shiftedTrialIEDoccurrence = ...
        zeros(0,size(channelIEDoccurrence,2));
    return;
end
...
trialRows = isfinite(channelIEDoccurrence(:,1)) & ...
    round(channelIEDoccurrence(:,1))==trialNumber & ...
    isfinite(channelIEDoccurrence(:,3)) & ...
    channelIEDoccurrence(:,3)>=1 & ...
    channelIEDoccurrence(:,3)<=maximumValidSample;
shiftedTrialIEDoccurrence = channelIEDoccurrence(trialRows,:);
if isempty(shiftedTrialIEDoccurrence), return; end

% The same participant-trial shift is used for every channel in the area.
% This preserves duplicate IEDs, temporal spacing, and cross-channel timing.
shiftInSamples = mod(round(shiftInSamples),maximumValidSample);
originalSamples = round(shiftedTrialIEDoccurrence(:,3));
shiftedSamples = mod(originalSamples-1+shiftInSamples, ...
