...
outputFolder = ...
    'D:\Nill\code\BART\IED\0_0_new_IED_new_area_labels\IED8_1_Cox_expected_reward_occurrence\';
...
%% Make one 4 x 2 figure

fig = figure('Visible', 'off', 'Color', 'w', ...
    'Position', [70 30 1500 2000]);
...
combinedPDFOutputFile = fullfile(outputFolder, ...
    'IT_RT_BR_PR_expected_reward_overall_cox_visualization_4x2.pdf');
...
function plotForest(ax, result, config)
    ...
    hold(ax, 'on');
    errorbar(ax, data.hazardRatio, y, ...
        data.hazardRatio - data.hazardRatioCILow, ...
        data.hazardRatioCIHigh - data.hazardRatio, ...
    ...
    set(ax, 'YTick', y, 'YTickLabel', labels, 'YDir', 'reverse', ...
        'XScale', 'log', 'FontName', 'Arial', 'FontSize', 10, ...
        'TickDir', 'out', ...
    ...
    for row = 1:height(data)
        if pValues(row) < 0.05
            text(ax, 1.06 * data.hazardRatioCIHigh(row), ...
...
function plotExpectedReward(ax, result, config, iedMode)
    ...
    resultText = sprintf([ ...
        'IED x reward HR multiplier = %.3f\n' ...
        '95%% CI [%.3f, %.3f]\n' ...
        'permutation p = %.3g\n' ...
        'HR per reward unit at IED 0 = %.3f\n' ...
        'HR per reward unit at IED 1 = %.3f'], ...
        row.hazardRatio, row.hazardRatioCILow, row.hazardRatioCIHigh, ...
        row.permutationPValue, exp(result.expectedRewardSlopeOutsideIED), ...
        exp(result.expectedRewardSlopeInsideIED));
