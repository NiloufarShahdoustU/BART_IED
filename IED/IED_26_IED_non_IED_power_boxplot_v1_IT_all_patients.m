% ccn 2026
% IT: power box plot (ALL PATIENTS, BASELINE-CORRECTED)
...
outputFolderName = '\\155.100.91.44\d\Code\Nill\BART\IED\IED_26_IED_non_IED_power_boxplot_v1_IT_all_patients\';
...
for e = 1:numel(epochNames)
    ...
    for a = 1:nAreas
        ...
        if isfield(areaDataAll.(areaSafe), epochName)
            ...
            % scatters (FIX: jitter vectors match dSig(:)/dNonSig(:))
            jitter = 0.18;
            dSigV    = dSig(:);
            dNonSigV = dNonSig(:);
...
sgtitle(sprintf('ALL PATIENTS | IT Band power (baseline-corrected within patient)'), 'Interpreter','none');

fnameBase = sprintf('ALLPATIENTS_IT_BoxplotBandPower_DELTAfromNonIED_%dto%dHz_2epochs_ALLareas_yTop%d_signrank_Holm', ...
