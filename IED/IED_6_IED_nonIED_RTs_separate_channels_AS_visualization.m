...
% for pt = 1:1
    ...
    pVal_periStimRT_filtered = IEDdata.pVal_periStimRTfiltered;
...
for pt = 1:PatientsNum
    ...
    for pre=1:length(AnatomicalLocsPre)
        ...
        % clear ChanIndicesPre FoundIndexInLocs nan_index tempIndexInLocs
    end
    ...
    for peri=1:length(AnatomicalLocsPeri)
        ...
        % clear ChanIndicesPeri FoundIndexInLocs nan_index tempIndexInLocs
    ...
    for post=1:length(AnatomicalLocsPost)
        ...
        % clear ChanIndicesPost FoundIndexInLocs nan_index tempIndexInLocs
    end
...
box off;
suptitle("Channels by brain area with significantly different RTs from Non-IED trials across patients");
...
figure('Units', 'normalized', 'Position', [0.1, 0, 0.18, 0.78]);

position1 = [0.22, 0.94, 0.775, 0.18];
position2 = [0.22, 0.56, 0.775, 0.18];
position3 = [0.22, 0.18, 0.775, 0.18]; 

threshold = 4; % Set to the minimum value that should be visible

% Pre plot
subplot('Position', position1);
visibleIndices = ChanNumsPre > threshold & ChanNumsPrePercent > 2;
...
% Peri plot
subplot('Position', position2);
visibleIndices = ChanNumsPeri > threshold & ChanNumsPeriPercent > 2;
...
% Post plot
subplot('Position', position3);
visibleIndices = ChanNumsPost > threshold & ChanNumsPostPercent > 1;
...
ylim([0 5]);
ylabel('mean RT across patients (s)', 'FontSize', 14, 'FontWeight', 'bold');
title('mean RT in three time periods of stimulus onset', 'FontSize', 16, 'FontWeight', 'bold');
...
if pValuePre < significanceThreshold
    text(2, max(allVecs) * 0.8, '*', 'FontSize', 24, 'HorizontalAlignment', 'center');
end
if pValuePeri < significanceThreshold
    text(3, max(allVecs) * 0.8, '*', 'FontSize', 24, 'HorizontalAlignment', 'center');
end
if pValuePost < significanceThreshold
    text(4, max(allVecs) * 0.8, '*', 'FontSize', 24, 'HorizontalAlignment', 'center');
end
