...
outputFolderName = '\\155.100.91.44\d\Data\Nill\BART\IEDdata_ACs\'; 
Fnew = 500;
...
%  I'm actually looking at banked trials.
    ...
    for row = 1:nChans-1
        for col = 1:nTrials
            ...
            if IEDtrials(row, col) == 0 
                NonIEDTrialsRTs(row, col) = BankedTrials(col);
            end
            ...
            if any(slice(:) == 1)
                IEDtrials_preStimRT(row, col) = BankedTrials(col);
            end
            ...
            slice(isnan(slice)) = 0; % Replace NaN with 0
            if any(slice(:) == 1)
                 IEDtrials_periStimRT(row, col) = BankedTrials(col);
            end
            ...
            slice(isnan(slice)) = 0; % Replace NaN with 0
            if any(slice(:) == 1)
                 IEDtrials_postStimRT(row, col) = BankedTrials(col);
            end
    ...
    for chz = start_channel:end_channel
        ...
        if (size(IEDtrials_preStimRT_temp_vec)>0)
            pVal_preStimRT(chz) = chiSquaredTest(IEDtrials_preStimRT_temp_vec,NonIEDTrials_temp_vec);
        else
            pVal_preStimRT(chz) = NaN;
        end

        if (size(IEDtrials_periStimRT_temp_vec)>0)
            pVal_periStimRT(chz) = chiSquaredTest(IEDtrials_periStimRT_temp_vec,NonIEDTrials_temp_vec);
        else
            pVal_periStimRT(chz) = NaN;
        end

        if (size(IEDtrials_postStimRT_temp_vec)>0)
            pVal_postStimRT(chz) = chiSquaredTest(IEDtrials_postStimRT_temp_vec,NonIEDTrials_temp_vec);
        else
