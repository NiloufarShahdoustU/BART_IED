...
% ATTENTION: all the RTs here are meant as ITs, I just copied and pasted
% the code and did not change the name of the variables. 
...
inputFolderName_IEDdata = '\\155.100.91.44\d\Data\Nill\BART\IEDdata_ITs';
...
b = bar(sortedValues, 'FaceColor', [0.329, 0.329, 0.306]); 
...
suptitle("Channels by brain area with significantly different ITs from Non-IED trials across patients");
...
figure('Units', 'normalized', 'Position', [0.1, 0, 0.21, 0.82]);

position1 = [0.22, 0.83, 0.775, 0.18];
position2 = [0.22, 0.5, 0.775, 0.18];
position3 = [0.22, 0.2, 0.775, 0.18]; 

threshold = 4; % Set to the minimum value that should be visible

% Pre plot
subplot('Position', position1);
visibleIndices = ChanNumsPre > threshold & ChanNumsPrePercent > 6;
...
ylim([0 25]);
text(max(xlim)-0.03*max(xlim), max(ylim)-2, 'pre stimulus onset', 'FontWeight', 'bold', 'FontSize', 14, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'top');

for i = 1:length(sortedValues)
    ChanNumInAll = find(AnatomicalLocsVecAll == sortedLabels(i)); 
    NumOfAllChans = ChanNumsAll(ChanNumInAll);
    labelText = sprintf('%d%% \n %d/%d', sortedValues(i), sortedChans(i), NumOfAllChans);
    text(i, sortedValues(i) + .5, labelText, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');
end
box off;

% Peri plot
subplot('Position', position2);
visibleIndices = ChanNumsPeri> threshold & ChanNumsPeriPercent > 6;
...
set(gca, 'XTickLabel', get(gca, 'XTickLabel'),  'FontSize', 10, 'FontWeight', 'bold');
ylim([0 20]);
text(max(xlim)-0.03*max(xlim), max(ylim)-2, 'peri stimulus onset', 'FontWeight', 'bold', 'FontSize', 14, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'top');
...
text(max(xlim)-0.03*max(xlim), max(ylim)-2, 'post stimulus onset', 'FontWeight', 'bold', 'FontSize', 14, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'top');
...
% Title and saving
suptitle(" ");
annotation('textbox', [0.05, 0.86, 0.95, 0.1], 'String', 'percentage of channels with significantly different IED and non-IED ITs', 'EdgeColor', 'none', 'HorizontalAlignment', 'center', 'FontWeight', 'bold', 'FontSize', 18);
annotation('textbox', [0.12, 0.42, 0.5, 0.06], 'String', 'channels across patients (%)', 'EdgeColor', 'none', 'HorizontalAlignment', 'center', 'FontWeight', 'bold', 'FontSize', 18, 'Rotation', 90);
...
ylim([0 max(allVecs)+3]);
ylabel('mean IT across patients (s)', 'FontSize', 14, 'FontWeight', 'bold');
title('mean IT in three time periods of stimulus onset', 'FontSize', 16, 'FontWeight', 'bold');
