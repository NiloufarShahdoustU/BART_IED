...
trodeLocs = fullfile('\\155.100.91.44\d','Data','preProcessed','BART_preprocessed',patientID,'Imaging','Registered','ChannelMap.mat');
...
if sanityCheck
    ...
    if exist(fullfile('\\155.100.91.44\d','Data','preProcessed','BART_preprocessed','electrodeLabels_wEEG',[patientID 'labels.csv']),'file')
        CSVlabels = table2cell(readtable(fullfile('\\155.100.91.44\d','Data','preProcessed','BART_preprocessed','electrodeLabels_wEEG',sprintf('%slabels.csv',patientID)),'ReadVariableNames',false));
    elseif exist(fullfile('\\155.100.91.44\d','Data','preProcessed','BART_preprocessed','electrodeLabels',[patientID 'labels.csv']),'file')
        % for micro labels:: usually no EEG. output of convertChanMapToElcetrodeLaberls.m
        CSVlabels = table2cell(readtable(fullfile('\\155.100.91.44\d','Data','preProcessed','BART_preprocessed','electrodeLabels',sprintf('%slabels.csv',patientID)),'ReadVariableNames',false));
    else
        % getting label data
        if exist('ChanMap','var')
