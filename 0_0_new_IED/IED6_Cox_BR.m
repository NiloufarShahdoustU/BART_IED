% Cause-specific Cox proportional hazards model for banking during full IT duration
%
%
% Author: Nill
...
outputFolderName = ...
    'D:\Nill\code\BART\IED\0_0_new_IED\IED6_Cox_BR\';
...
for pt = 1:length(fileList)
    ...
    requiredFields = {'RTs', 'ITs', 'isControl', 'balloonType', ...
        'BankedTrials', 'IED_occurance_IT'};
    ...
    nTrials = determineNumberOfTrials( ...
        LFPIED, RTs, ITs, isControl, balloonType, BankedTrials);
...
for ii = 1:length(beta)
    ...
    if ii == 1
        interpretation(ii) = ...
            "hr > 1 means a higher instantaneous banking hazard; " + ...
            "hr < 1 means a lower instantaneous banking hazard. " + ...
            "non-banked/popped trials are censored at the end of it.";
    else
        interpretation(ii) = ...
...
if iedRow.pValue < 0.05
    ...
    if iedRow.hazardRatio > 1
        fprintf(['conclusion: more prior ieds are associated with a ' ...
            'significantly higher instantaneous banking hazard.\n']);
    else
        fprintf(['conclusion: more prior ieds are associated with a ' ...
            'significantly lower instantaneous banking hazard.\n']);
    end

else
    fprintf(['conclusion: the association between prior ieds and the ' ...
        'instantaneous banking hazard is not statistically significant.\n']);
end
...
figurePDFOutputFile = fullfile( ...
    outputFolderName, 'BR_cox_model_visualization.pdf');

figurePNGOutputFile = fullfile( ...
    outputFolderName, 'BR_cox_model_visualization.png');

effectCurveOutputFile = fullfile( ...
    outputFolderName, 'BR_IED_model_effect_curve.csv');
...
countingProcessOutputFile = fullfile( ...
    outputFolderName, 'BR_counting_process_data.csv');

trialSummaryOutputFile = fullfile( ...
    outputFolderName, 'BR_trial_summary.csv');

coxResultsOutputFile = fullfile( ...
    outputFolderName, 'BR_cox_results.csv');

baselineHazardOutputFile = fullfile( ...
    outputFolderName, 'BR_baseline_cumulative_hazard.csv');

modelOutputFile = fullfile( ...
    outputFolderName, 'BR_cox_model.mat');
...
function [fig, effectCurveTable] = plotCoxModelVisualization( ...
    ...
    colorBRLight = 0.75 .* [1 1 1] + 0.25 .* colorBR;
    ...
    errorbar( ...
        ...
        'LineWidth', 1.2, ...
        'MarkerSize', 6, ...
        'MarkerFaceColor', colorBR, ...
        'MarkerEdgeColor', colorBR, ...
        'Color', colorBR, ...
        'CapSize', 7);

    xline(1, '--', 'Color', colorBR, 'LineWidth', 1);
    ...
    %% Panel 2: primary IED banking-hazard effect
    ...
    yline(1, '--', 'Color', colorBR, 'LineWidth', 1);
    ...
    ylabel('relative banking hazard', 'Color', 'k');
