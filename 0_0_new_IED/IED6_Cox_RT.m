% Cox proportional hazards model for full RT duration
%
% Participant-stratified Cox proportional hazards model with a
% time-varying IED predictor over the entire RT duration.

%   h_i(t) = h_0,patient(t) * exp(beta_IED * IED_timevarying_i(t) + balloon-color covariates)
...
outputFolderName = ...
    'D:\Nill\code\BART\IED\0_0_new_IED\IED6_Cox_RT\';
...
for ii = 1:length(beta)
    ...
    if ii == 1
        interpretation(ii) = ...
            "hr > 1 means a higher response hazard and therefore shorter rt; " + ...
            "hr < 1 means a lower response hazard and therefore longer rt.";
    else
        interpretation(ii) = ...
...
fprintf('\n============================================================\n');
fprintf('full-duration rt cox model\n');
fprintf('============================================================\n');
...
if iedRow.pValue < 0.05
    ...
    if iedRow.hazardRatio > 1
        fprintf(['conclusion: more prior ieds are associated with a ' ...
            'significantly higher response hazard, corresponding to ' ...
            'shorter rt.\n']);
    else
        fprintf(['conclusion: more prior ieds are associated with a ' ...
            'significantly lower response hazard, corresponding to ' ...
            'longer rt.\n']);
    end

else
    fprintf(['conclusion: the association between prior ieds and rt ' ...
        'response hazard is not statistically significant.\n']);
end
...
figurePDFOutputFile = fullfile( ...
    outputFolderName, 'RT_cox_model_visualization.pdf');

figurePNGOutputFile = fullfile( ...
    outputFolderName, 'RT_cox_model_visualization.png');

effectCurveOutputFile = fullfile( ...
    outputFolderName, 'RT_IED_model_effect_curve.csv');
...
countingProcessOutputFile = fullfile( ...
    outputFolderName, 'RT_counting_process_data.csv');

trialSummaryOutputFile = fullfile( ...
    outputFolderName, 'RT_trial_summary.csv');

coxResultsOutputFile = fullfile( ...
    outputFolderName, 'RT_cox_results.csv');

baselineHazardOutputFile = fullfile( ...
    outputFolderName, 'RT_baseline_cumulative_hazard.csv');

modelOutputFile = fullfile( ...
    outputFolderName, 'RT_cox_model.mat');
...
function trialRows = makeCountingProcessRows( ...
    ...
    %   1 = trial
    %   2 = channel
    %   3 = sample index within the RT interval
...
function [fig, effectCurveTable] = plotCoxModelVisualization( ...
    ...
    colorRTLight = 0.75 .* [1 1 1] + 0.25 .* colorRT;
    ...
    xline(1, '--', 'Color', colorRT, 'LineWidth', 1);
    ...
    yline(1, '--', 'Color', colorRT, 'LineWidth', 1);
    ...
    ylabel('relative response hazard', 'Color', 'k');
