...
% matter labels are excluded before fitting, correction, saving, and plotting.
%
% Outputs:
...
function [areaResults, areaModels] = runBrainAreaAnalysis( ...
    ...
    analysisPrefix = analysisType;
    postIEDWindowMilliseconds = settings.postIEDWindowMilliseconds;
    postIEDWindowSeconds = settings.postIEDWindowSeconds;
    ...
    for pt = 1:length(fileList)
        ...
        % Exclude all NaC/nucleus accumbens and white-matter channels.
    ...
    if any(excludedResultRows)
        fprintf([ ...
            '\nHard-removing %d NaC/white-matter result rows ' ...
    ...
    if any(fittedRows)
        ...
        areaResults.pValueBonferroni(fittedRows) = min( ...
            areaResults.pValue(fittedRows) * numberOfFittedAreas, 1);
...
function [X, predictorNames, referenceColorName] = ...
    ...
    predictorNames = "active_area_ied_count_within_500ms";
...
function plotCombinedAreaForestPlots( ...
    ITResults, RTResults, BRResults, ...
    colorIT, colorRT, colorBR, markerAlpha, outputPDF)
...
function plotAreaForestPanel( ...
    ...
    title(ax, {char(descriptiveTitle), char(abbreviation)}, ...
        'FontWeight', 'bold');
