% Cause-specific Cox proportional hazards model for banking during IT
%
...
outputFolderName = ...
    'D:\Nill\code\BART\IED\0_0_new_IED\IED7_Cox_BR_postIED500ms\';
...
for pt = 1:length(fileList)
    ...
    requiredFields = {'RTs', 'ITs', 'isControl', 'balloonType', 'BankedTrials', 'IED_occurance_IT'};
...
for ii = 1:length(beta)
    ...
    if ii == 1
        interpretation(ii) = ...
            ...
            "higher instantaneous banking hazard for each additional ied; " + ...
            "hr < 1 means lower banking hazard. non-banked or popped trials " + ...
            "are censored at the end of it.";
    else
        interpretation(ii) = ...
...
fprintf('\n============================================================\n');
fprintf('500 ms post-ied cause-specific banking cox model\n');
fprintf('============================================================\n');
...
if primaryRow.pValue < 0.05
    ...
    if primaryRow.hazardRatio > 1
        fprintf('conclusion: each additional active ied window is associated with a significantly higher instantaneous banking hazard.\n');
    else
        fprintf('conclusion: each additional active ied window is associated with a significantly lower instantaneous banking hazard.\n');
    end

else
    fprintf('conclusion: the instantaneous banking hazard does not significantly change per additional active ied window.\n');
end
...
figurePDFOutputFile = fullfile( ...
    outputFolderName, 'BR_postIED500ms_cox_visualization.pdf');

figurePNGOutputFile = fullfile( ...
    outputFolderName, 'BR_postIED500ms_cox_visualization.png');

modelEffectOutputFile = fullfile( ...
    outputFolderName, 'BR_postIED500ms_model_effect.csv');

patientRatesOutputFile = fullfile( ...
    outputFolderName, 'BR_postIED500ms_patient_event_rates.csv');
...
countingProcessOutputFile = fullfile( ...
    outputFolderName, 'BR_postIED500ms_counting_process_data.csv');

trialSummaryOutputFile = fullfile( ...
    outputFolderName, 'BR_postIED500ms_trial_summary.csv');

coxResultsOutputFile = fullfile( ...
    outputFolderName, 'BR_postIED500ms_cox_results.csv');

baselineHazardOutputFile = fullfile( ...
    outputFolderName, 'BR_postIED500ms_baseline_cumulative_hazard.csv');

modelOutputFile = fullfile( ...
    outputFolderName, 'BR_postIED500ms_cox_model.mat');
...
function [fig, modelEffectTable] = plotPostIEDVisualization( ...
    ...
    colorBR = [0.250  0.600  0.250];   % requested color
    lightColor = 0.75 .* [1 1 1] + 0.25 .* colorBR;
    ...
    ylabel('relative banking hazard vs 0 active ieds', 'Color', 'k');
    ...
    ylabel('banking events per 100 s at risk', 'Color', 'k');
