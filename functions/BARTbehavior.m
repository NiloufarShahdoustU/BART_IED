...
matFile = ['\\155.100.91.44\d\Data\preProcessed\BART_preprocessed\' ptID '\Data\' ptID '.bartBHV.mat']; % if working on workstation
...
parentDir = ['\\155.100.91.44\d\Data\preProcessed\BART_preprocessed\' ptID '\Data\*.nev']; % if working on workstation
...
pTrialBanked = [false, banked(1:end-1)]; % previous trial banked
pTrialPopped = [false, popped(1:end-1)]; % previous trial poped
nPopped = sum(popped);
...
poppedRTs = allRTs(popped & ~isCtrl & ~outRTs); % Response times for unsuccessful trials (popped)
...
%  previous trial was these baloons and they poped or banked
...
% the response times for the trials that their previous trials were each of
% these balloons and they poped or banked
...
bhvStruct.yellowBanked = yellowBanked;
bhvStruct.orangeBanked = orangeBanked;
bhvStruct.redBanked = redBanked;
bhvStruct.isCtrl = isCtrl;
...
%% saving stats
save(['\\155.100.91.44\d\Data\Nill\BART\bhvStruct_Nill_made\' ptID '.bhvStruct.mat'],'bhvStruct')
